...
def remove_tmp_files(del_sub_dirs,**template_dict):
    ...
    if os.path.isfile(os.path.join(del_sub_dirs,template_dict['qa_flagged_filename'])):
        os.remove(os.path.join(del_sub_dirs,template_dict['qa_flagged_filename']))

    [shutil.rmtree(os.path.join(del_sub_dirs,each_dir), ignore_errors=True) for each_dir in list(filter(lambda x: os.path.isdir(os.path.join(del_sub_dirs, x)), os.listdir(del_sub_dirs))) ]
...
def run_pipeline(write_dir,
    ...
    vbm_output_files=list() # List for storing output file names for json out
    ...
    for each_sub in smri_data:
        ...
        try:
            ...
            template_dict['covariates'][loop_counter][0]=segmented_file
        finally:
            shutil.copy(
                os.path.join(vbm_out, template_dict['vbm_output_dirname'],
                             template_dict['regression_file']),
                os.path.join(os.path.dirname(write_dir),
                             sub_id + '_' + (template_dict['regression_file']).split('.')[0] + '_' +template_dict['vbm_append_string'])+'.nii')

            vbm_output_files.append(sub_id + '_' + (template_dict['regression_file']).split('.')[0] + '_' +template_dict['vbm_append_string']+'.nii')
    ...
    remove_tmp_files(os.path.dirname(write_dir), **template_dict)

    return json.dumps({
        "output": {
            str(template_dict['covariates'])
        },
        "cache": {},
        "success": True
    })
